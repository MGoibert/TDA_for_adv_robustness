...
def threshold_sig(x, k="auto", mean=0, quant=1):
    if k == "auto":
        k = -1/(quant-mean) * np.log(0.01/0.99)
    return 1/(1+np.exp(-k*(x-mean)))

means = [37831, 0, 94527, 0, 100438, 241302, 981380]
quants = [166841, 1000000, 542584, 1000000, 592224, 1351523, 6186379]
...
if args.attack_type in ["FGSM", "BIM"]:
    all_epsilons = list([0.0, 0.005, 0.01, 0.02])
else:
    all_epsilons = [0.0, 1]

# Adv train dataset
logger.info(f"Adv train dataset !!")
adv_embeddings_train = get_embeddings(epsilon=args.noise, noise=0.0, start=start)
start += args.dataset_size

labels_train = np.concatenate(
            (
                np.zeros(len(clean_embeddings_train)),
                np.zeros(len(noisy_embeddings_train)),
                np.ones(len(adv_embeddings_train))
            )
        )
...
gram_train_matrices = {i: get_gram_matrix(
    ...
    embeddings_in=clean_embeddings_train + noisy_embeddings_train + adv_embeddings_train,
    embeddings_out=None,
    params=param
)
...
def process_epsilon(epsilon: float) -> float:
    ...
    for i, param in enumerate(param_space):
        ...
        gram_test_and_bad = get_gram_matrix(
            ...
            embeddings_out=clean_embeddings_train + noisy_embeddings_train + adv_embeddings_train,
            params=param
        )
        ...
        labels = np.concatenate(
            (
                np.zeros(len(clean_embeddings_test)),
                np.zeros(len(noisy_embeddings_test)),
                np.ones(len(adv_embeddings[epsilon]))
            )
        )
        logger.info(f"Score : {svc.score(gram_test_and_bad, labels)}")
        logger.info(f"Confusion matrix = {confusion_matrix(labels, svc.predict(gram_test_and_bad))}")

        roc_auc_val = roc_auc_score(y_true=labels, y_score=predictions[:, 1])
